<x-guest-layout>
    <x-authentication-card>
        <h2 class="flex mt-8 font-semibold text-xl my-4 text-blue-800 leading-tight">
            Registro de cuenta
        </h2>
        <x-slot name="logo">
        ...
        <form method="POST" action="{{ route('register') }}">
            @csrf
            <div class="grid grid-cols-2 gap-4 register">
                <div>
                    <x-label for="name" value="{{ __('Nombre') }}" />
                    <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="firstname" />
                </div>
                <div>
                ...
                <div class="col-span-2">
                    ...
                    <x-textarea id="address" class="block  resize-none mt-1 w-full" type="text" name="address" :value="old('address')" required autofocus autocomplete="address" />
                </div>
                <div class="col-span-2">
                    <livewire:ciudades wire:model="city_id" id="city_id" name="city_id" :value="old('city_id')" required />
                </div>
                <div>
